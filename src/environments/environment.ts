// This file can be replaced during build by using the `fileReplacements` array.
// `ng build --prod` replaces `environment.ts` with `environment.prod.ts`.
// The list of file replacements can be found in `angular.json`.

export const environment = {
  production: false,
  config : {
     apiKey: "AIzaSyBAgkgEVNAGRtyCze9UekpthPnebDUu_wM",
    authDomain: "psl-project.firebaseapp.com",
    databaseURL: "https://psl-project.firebaseio.com",
    projectId: "psl-project",
    storageBucket: "psl-project.appspot.com",
    messagingSenderId: "517232940663"
	
	/*apiKey: 'AIzaSyAnotfaQmw5cJMJxQX-04fQ4YksZZWRDjQ',
    authDomain: 'ionic4fullapp.firebaseapp.com',
    databaseURL: 'https://ionic4fullapp.firebaseio.com',
    projectId: 'ionic4fullapp',
    storageBucket: 'ionic4fullapp.appspot.com',
    messagingSenderId: '124018728460' */
  }
};

/*
 * For easier debugging in development mode, you can import the following file
 * to ignore zone related error stack frames such as `zone.run`, `zoneDelegate.invokeTask`.
 *
 * This import should be commented out in production mode because it will have a negative impact
 * on performance if an error is thrown.
 */
// import 'zone.js/dist/zone-error';  // Included with Angular CLI.
